...
static void coordinate_transfer(PointXYZIT *point, uint32_t coordinate_mode, float x, float y, float z) {
...
void SeyondDriver::setup_param() {
  ...
  if (conf_.has_pointcloud_channel()) pointcloud_channel_ = conf_.pointcloud_channel();
  ...
  if (conf_.has_reflectance_mode()) reflectance_mode_ = conf_.reflectance_mode();
...
bool SeyondDriver::init_lidar() {
  ...
  enum InnoLidarProtocol protocol_ = (udp_port_ >= 0 ? INNO_LIDAR_PROTOCOL_PCS_UDP : INNO_LIDAR_PROTOCOL_PCS_TCP);
  handle_ = inno_lidar_open_live(lidar_name_.c_str(), ip_str, port_, protocol_, lidar_udp_port);
  // get internal pcs enable status
  uint32_t is_internal_pcs = 1;
  char buf[20] = {'\0'};
  int32_t ret = inno_lidar_get_attribute_string(handle_, "enabled", buf, sizeof(buf));
  if (ret != 0) {
  ...
  if (is_internal_pcs != 1) {
    inno_log_error("error getting PCS enable status: %u, exiting...", is_internal_pcs);
    return false;
  }
  std::string cilent_set_key = "";
  cilent_set_key = "LidarClient_Communication/get_conn_timeout_sec";
  ret = inno_lidar_set_config_name_value(handle_, cilent_set_key.c_str(), "5.0");
  if (ret != 0) {
...
int32_t SeyondDriver::set_input_parameter_() {
  ...
  ret = inno_lidar_set_return_mode(handle_, (InnoMultipleReturnMode)multiple_return_);
  if (ret != 0) {
  ...
  // set lidar callback
  ret = inno_lidar_set_callbacks(handle_, message_callback_s_, data_callback_s_, status_callback_s_,
...
bool SeyondDriver::init() {
  ...
  scan_reader_ptr_ = node_ptr_->CreateReader<SeyondScan>(scan_channel_,
     std::bind(&SeyondDriver::process_scan_packet_, this, std::placeholders::_1));
...
// callback group
void SeyondDriver::message_callback_(uint32_t from_remote, enum InnoMessageLevel level, enum InnoMessageCode code,
                        const char *msg) {
  const char *remote = "";
  if (from_remote) {
    remote = "REMOTE-";
...
void SeyondDriver::log_callback_(enum InnoLogLevel level, const char *header2, const char *msg) {
  switch (level) {
...
int32_t SeyondDriver::data_callback_(const InnoDataPacket *pkt) {
  ...
  if ((pkt->type == INNO_ROBINW_ITEM_TYPE_COMPACT_POINTCLOUD) && (!anglehv_table_init_)) {
    ...
    int ret = inno_lidar_get_anglehv_table(handle_, reinterpret_cast<InnoDataPacket*>(anglehv_table_.data()));
    if (ret == 0) {
...
int32_t SeyondDriver::process_packets_and_publish_frame_(const InnoDataPacket *pkt) {
  ...
  if ((current_frame_id_ != static_cast<int64_t>(pkt->idx))) {
    ...
    point_cloud_ptr_->mutable_header()->set_sequence_num(current_frame_id_ % UINT_MAX);
    point_cloud_ptr_->mutable_header()->set_lidar_timestamp(((uint64_t)(pkt->common.ts_start_us)) * 1000);
...
int32_t SeyondDriver::collect_packets_and_publish_packets_(const InnoDataPacket *pkt) {
  // check if get the whole frame, and publish
  if (current_frame_id_ != static_cast<int64_t>(pkt->idx)) {
    frame_count_++;
    scan_packets_ptr_->set_sequence_num(current_frame_id_ % UINT_MAX);
    scan_packets_ptr_->set_timestamp(((uint64_t)(pkt->common.ts_start_us)) * 1000);
  ...
  if (anglehv_table_init_ && (frame_count_ == table_send_hz_)) {
    ...
    scan_packet->mutable_table()->assign(anglehv_table_.data(), anglehv_table_.size());
  }
  packets_width_++;
...
int32_t SeyondDriver::process_scan_packet_(const std::shared_ptr<SeyondScan>& lidar_packets) {
  for (auto &packet : lidar_packets->packets()) {
    const InnoDataPacket *pkt = reinterpret_cast<const InnoDataPacket *>(packet.data().data());
    if ((raw_packets_mode_) && (!anglehv_table_init_) && (pkt->type == INNO_ROBINW_ITEM_TYPE_COMPACT_POINTCLOUD)) {
      if (packet.table_exist()) {
        anglehv_table_init_ = true;
        anglehv_table_.resize(sizeof(InnoAngleHVTable) + sizeof(InnoDataPacket));
        std::memcpy(anglehv_table_.data(), packet.table().data(), sizeof(InnoAngleHVTable) + sizeof(InnoDataPacket));
        inno_log_info("Get RobinW Compact Table");
      } else {
        return 0;
      }
  ...
  if (lidar_packets->is_last_scan()) {
    point_cloud_ptr_->mutable_header()->set_frame_id(frame_id_);
    point_cloud_ptr_->mutable_header()->set_sequence_num(lidar_packets->sequence_num());
    point_cloud_ptr_->mutable_header()->set_lidar_timestamp(lidar_packets->timestamp());
    point_cloud_ptr_->set_frame_id(frame_id_);
  ...
  if (CHECK_SPHERE_POINTCLOUD_DATA(pkt->type)) {
    ...
    if (anglehv_table_init_) {
      inno_lidar_convert_to_xyz_pointcloud2(
          pkt, reinterpret_cast<InnoDataPacket *>(&convert_buffer_[0]), convert_buffer_.size(), false,
          reinterpret_cast<InnoDataPacket *>(anglehv_table_.data()));
    } else {
      inno_lidar_convert_to_xyz_pointcloud(pkt, reinterpret_cast<InnoDataPacket *>(&convert_buffer_[0]),
                                           convert_buffer_.size(), false);
    }
    process_data_packet_(reinterpret_cast<InnoDataPacket *>(&convert_buffer_[0]));
...
int32_t SeyondDriver::process_data_packet_(const InnoDataPacket *pkt) {
  ...
  if (CHECK_EN_XYZ_POINTCLOUD_DATA(pkt->type)) {
    ...
    process_xyz_point_data_<const InnoEnXyzPoint *>(true, pkt->use_reflectance, pkt->item_number, pt);
  } else {
    const InnoXyzPoint *pt =
      reinterpret_cast<const InnoXyzPoint *>(reinterpret_cast<const char *>(pkt) + sizeof(InnoDataPacket));
    process_xyz_point_data_<const InnoXyzPoint *>(false, pkt->use_reflectance, pkt->item_number, pt);
  }
...
void SeyondDriver::process_xyz_point_data_(bool is_en_data, bool is_use_refl, uint32_t point_num,
                                               PointType point_ptr) {
  for (uint32_t i = 0; i < point_num; i++, point_ptr++) {
    if (point_ptr->radius == 0 || point_ptr->radius > max_range_ || point_ptr->radius < min_range_) {
      continue;
    }
    ...
    point->set_timestamp((uint64_t)(point_ptr->ts_10us / ten_us_in_second_c + current_ts_start_));
