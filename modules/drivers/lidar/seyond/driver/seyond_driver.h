...
class SeyondDriver {
 public:
  ...
  static void message_callback_s_(int32_t handle_, void *ctx, uint32_t from_remote, enum InnoMessageLevel level,
                                  enum InnoMessageCode code, const char *error_message) {
    (reinterpret_cast<SeyondDriver *>(ctx))->message_callback_(from_remote, level, code, error_message);
  }
  ...
  static void log_callback_s_(void *ctx, enum InnoLogLevel level, const char *header1, const char *header2,
                              const char *msg) {
  ...
  void message_callback_(uint32_t from_remote, enum InnoMessageLevel level, enum InnoMessageCode code,
                         const char *msg);
  int32_t status_callback_(const InnoStatusPacket *pkt);

  void set_log_and_message_level();
  void log_callback_(enum InnoLogLevel level, const char *header2, const char *msg);
  ...
  int32_t process_scan_packet_(const std::shared_ptr<SeyondScan>& lidar_packets);
